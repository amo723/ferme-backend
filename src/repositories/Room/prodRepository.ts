...
class ProdRoomRepository extends RoomRepository {
  async getSessionCookies() {
    ...
    try {
      ...
      const response = await fetch(
        `${NC_BASE_URL}/ocs/v2.php/apps/spreed/api/v1/signaling/session`,
        requestOptions
      );
  ...
  async createUser(
    ...
    if (response.status === 400 || response.ok) {
      ...
      throw new BadResponse(
        `Impossible de créer un utilisateur (${response.status})`,
        "TALK"
      );
    }
  }
  ...
  async createRoom(name: string = "ocare"): Promise<Room> {
    ...
    const room: Room = await fetch(`${TALK_BASE_URL}/room`, requestOptions)
      .then((response) => {
        ...
        throw new BadResponse(
          `Impossible de créer une room (${response.status})`,
          "TALK"
        );
      })
      .then((result) => {
  ...
  async addUserInRoom(token_room: string, user_id: string): Promise<void> {
    ...
    await fetch(
      `${TALK_BASE_URL}/room/${token_room}/participants`,
      requestOptions
    ).then((response) => {
      if (response.ok) {
        return response.json();
      ...
      throw new BadResponse(
        `Impossible d'ajouter l'utilisateur à la room (${response.status})`,
        "TALK"
      );
    });
  }
  ...
  async getRelatedRooms(
    ...
    await fetch(`${TALK_BASE_URL}/room`, requestOptions)
      .then((response) => {
        ...
        throw new BadResponse(
          `Impossible de recupérer les rooms liées à l'utilisateur (${response.status})`,
          "TALK"
        );
      })
      .then((result) => {
  ...
  async setRoomLinkable(token_room: string): Promise<void> {
    ...
    await fetch(
      `${TALK_BASE_URL}/room/${token_room}/public`,
      requestOptions
    ).then((response) => {
      if (response.ok) {
        return response.json();
      ...
      throw new BadResponse(
        `Impossible de rendre une room joignable par lien (${response.status})`,
        "TALK"
      );
    });
  }
  ...
  async setPasswordLinkedRoom(
    ...
    await fetch(
      `${TALK_BASE_URL}/room/${token_room}/password`,
      requestOptions
    ).then((response) => {
      if (response.ok) {
        return response.json();
      ...
      throw new BadResponse(
        `Impossible de mettre à jour le mot de passe du lien d'une room (${response.status})`,
        "TALK"
      );
    });
  }
  ...
  async getRoomParticipants(token_room: string): Promise<Array<User>> {
    ...
    await fetch(
      ...
      .then((response) => {
        ...
        throw new BadResponse(
          `Impossible de recupérer les participants (${response.status})`,
          "TALK"
        );
      })
      .then((result) => {
